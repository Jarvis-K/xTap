...
function findInstructions(endpoint, data) {
  ...
  if (path) {
    ...
    // Known endpoint but path failed — log where it broke
    let current = data;
    for (let i = 0; i < path.length; i++) {
      if (current == null || typeof current !== 'object') {
        console.warn(`[xTap:parser] Path broke for ${endpoint} at step ${i} ("${path[i]}") | parent keys: ${Object.keys(navigatePath(data, path.slice(0, i)) || {}).join(', ')}`);
        break;
      }
